...
import { getCountyRowColor, getSortIcon, filterDataByStatus, sortData } from './utils/helpers.ts';
import { ComparisonData, BaselineData } from './types';
...
const FirstandSecond: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/baseline-combined.json');
      if (!response.ok) {
