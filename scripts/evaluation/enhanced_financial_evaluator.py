...
class EnhancedKJETEvaluator:
    ...
    def find_financial_documents(self, bundle_path):
        """Find financial documents while skipping non-financial files"""
        financial_docs = []
        
        # Refined skip list for latest cohort / improved accuracy
        skip_patterns = [
            'application_info', 
            ...
            'compliance',
            'kjet_forms',
            'application_kjet',  # C2 Form
            'supporting_documents_registration',  # C2 Registration
            'memo_and_articles',
            'cr12',
            'cr13'
        ]
        ...
        for file in bundle_path.glob("*.pdf"):
            ...
            if any(fin_type in filename_lower for fin_type in [
                'mpesa', 'bank', 'financial', 'statement', 'income', 'balance', 'cashflow', 'ledger'
            ]):
...
def main():
    ...
    if args.output_dir:
        ...
        output_path = Path(__file__).resolve().parent.parent.parent / "output-results" / args.cohort / "financial"
    
    # Create and run evaluator
    evaluator = EnhancedKJETEvaluator(data_path, output_path)
